    ...
    function cycle() {
      ...
      delays.forEach((delay, i) => {
        timersRef.current.push(
          setTimeout(() => { cells[i]?.classList.add('is-on'); }, delay),
        );
      });
      ...
      cycleTimerRef.current = setTimeout(() => {
        ...
        delays.forEach((delay, i) => {
          timersRef.current.push(
            setTimeout(() => { cells[i]?.classList.remove('is-on'); }, delay),
          );
        });
    ...
    <div ref={containerRef} className={containerClassName} style={containerStyle}>
      {Array.from({ length: 9 }, (_, i) => {
        ...
        const cellClassName = [
          ...
          .join(' ');
        const cellStyle =
          cellColor && isHex(cellColor) ? hexVars(cellColor) : undefined;
